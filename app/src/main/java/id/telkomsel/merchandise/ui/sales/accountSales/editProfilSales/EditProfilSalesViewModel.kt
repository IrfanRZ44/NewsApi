package id.telkomsel.merchandise.ui.sales.accountSales.editProfilSales
...
class EditProfilSalesViewModel(
    ...
    private val editTglPeresmianSales: TextInputLayout,
    ...
    val etTglPeresmianSales = MutableLiveData<String>()
...
//        spinnerKategori.adapter = adapterKategori
    ...
    fun setDataSales() {
//        etEmailSales.value = dataSales?.email_sales
...
//        etTglPeresmianSales.value = dataSales?.tgl_peresmian_sales
        ...
        try {
            datePickerDialog = DatePickerDialog(
                ...
                { _, paramAnonymousInt1, paramAnonymousInt2, paramAnonymousInt3 ->
                    ...
                    dateSelected[paramAnonymousInt1, paramAnonymousInt2] = paramAnonymousInt3
                    val dateFormatter = SimpleDateFormat(Constant.dateFormat1, Locale.US)
                    etTglPeresmianSales.value = dateFormatter.format(dateSelected.time)
                },
                localCalendar[Calendar.YEAR],
                localCalendar[Calendar.MONTH],
                localCalendar[Calendar.DATE]
...
//
////                        val idKategori = dataSales?.kategori_id
////                        if (dataSales != null) {
...
//            })
    ...
    private fun setNullError() {
        ...
        editTglPeresmianSales.error = null
    ...
    fun onClickRegisterSales() {
        ...
        val tglPeresmianSales = etTglPeresmianSales.value
        ...
        if (dataSales != null && id != null && !namaSales.isNullOrEmpty() && !alamatSales.isNullOrEmpty() && !lat.isNullOrEmpty() && !lng.isNullOrEmpty()
            ...
            && !namaLengkap.isNullOrEmpty() && !tglLahir.isNullOrEmpty() && !tglPeresmianSales.isNullOrEmpty()
...
//            dataSales.tgl_peresmian_sales = tglPeresmianSales
...
//            updateSales(dataSales)
            ...
            if (dataSales == null) {
            ...
            } else if (tglPeresmianSales.isNullOrEmpty()) {
                message.value = "Error, Mohon pilih tanggal peresmian merchandise"
                editNamaSales.clearFocus()
                editAlamatSales.clearFocus()
                editTitikLokasi.clearFocus()
                editTglPeresmianSales.requestFocus()
                editTglPeresmianSales.findFocus()
                editTglPeresmianSales.error = "Error, Mohon pilih tanggal peresmian merchandise"
                ...
                setTextError(
                    "Error, mohon masukkan nomor HP Sales dengan awalan 0",
                    editNoHpSales
                )
                ...
                setTextError(
                    "Error, mohon masukkan nomor WA Sales dengan awalan 0",
                    editNoWaSales
                )
                ...
                override fun onResponse(
                    ...
                    if (result?.message == Constant.reffSuccessRegister) {
                        ...
                        if (result?.message == "Nomor HP Sales sudah digunakan") {
                            setTextError(
                                "Error, nomor HP Sales sudah digunakan",
                                editNoHpSales
                            )
                        } else {
