...
class AdminFragment : BaseFragmentBind<FragmentAdminBinding>() {
    ...
    private fun setupViewPager(pager: ViewPager) {
        ...
        adapter.addFragment(Blank1Fragment(), Constant.appName)
        ...
        bind.tabs.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                bind.tabs.getTabAt(0)?.icon = resources.getDrawable(R.drawable.ic_profile_black)
                ...
                when (tab.position) {
                    0 -> {
                        ...
                        bind.tabs.getTabAt(1)?.icon?.setColorFilter(resources.getColor(R.color.gray2), PorterDuff.Mode.SRC_IN)
                    }
                    else -> {
                        supportActionBar?.title = Constant.akun
                        bind.tabs.getTabAt(0)?.icon?.setColorFilter(resources.getColor(R.color.gray2), PorterDuff.Mode.SRC_IN)
