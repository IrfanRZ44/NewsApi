...
class AddMerchantViewModel(
    ...
    fun onClickRegisterMerchant(){
        ...
        if (!namaMerchant.isNullOrEmpty() && !alamatMerchant.isNullOrEmpty() && !lat.isNullOrEmpty() && !lng.isNullOrEmpty()
            ...
            validateMerchant(resultMerchant, fotoProfil)
        }
        else{
    ...
    private fun validateMerchant(resultMerchant: ModelMerchant, fotoProfil: String){
            ...
            object : Callback<ModelResponse> {
                override fun onResponse(
                    ...
                    if (result?.message == Constant.reffDataCanBeUsed) {
                        createMerchant(resultMerchant, fotoProfil)
                    } else {
                        when (result?.message) {
    ...
    private fun createMerchant(resultMerchant: ModelMerchant, fotoProfil: String){
        RetrofitUtils.createMerchant(resultMerchant,
            object : Callback<ModelResponse> {
                override fun onResponse(
                    call: Call<ModelResponse>,
                    response: Response<ModelResponse>
                    ...
                    if (result?.message == Constant.reffSuccessRegister){
                        ...
                        val nameFile = "${resultMerchant.username}__${System.currentTimeMillis()}"

                        activity?.let { RetrofitUtils.uploadFoto(Constant.dataModelFotoProfil,
                            Constant.folderFotoProfil, nameFile, resultMerchant.username, fotoProfil, it) }
                        ...
                        if (!msgErr.isNullOrEmpty()){
                            when {
                                msgErr.contains("Duplicate entry '${resultMerchant.username}' for key 'username'") -> {
                                    message.value = "Username Sudah Digunakan"
                                }
                                msgErr.contains("Duplicate entry '${resultMerchant.no_hp_merchant}' for key 'no_hp_merchant'") -> {
    ...
    private fun dialogSucces() {
        if (activity != null){
            ...
            alert.setMessage("Berhasil mendaftarkan Merchant, mohon tunggu proses verifikasi dalam waktu 1x24 jam")
