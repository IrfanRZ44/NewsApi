...
class PesanAdmintoLDViewModel(private val rcChat: RecyclerView,
    ...
    private var adapter: AdapterPesanAdmintoLD? = null
    ...
    fun setData() {
        ...
        if (dataLembaga.value?.namaPanjangLembaga.isNullOrEmpty()){
            ...
            activity?.actionBarName?.text = dataLembaga.value?.namaPanjangLembaga
        }
        ...
        activity?.contact_photo?.load(dataLembaga.value?.fotoLembaga) {
            crossfade(true)
        ...
        activity?.contact_photo?.setOnClickListener {
            dataLembaga.value?.fotoLembaga?.let { it1 -> onClickFoto(it1, navController) }
        }
    }

    fun initAdapter() {
        adapter = AdapterPesanAdmintoLD(listChat,
        ...
        val valueEventListener = object : ValueEventListener {
            ...
            override fun onDataChange(result: DataSnapshot) {
                ...
                if (result.exists()) {
                    ...
                    data?.idLembaga?.let { getDataLembaga(it) }
                    setData()
                } else {
    ...
    fun onClickSend() {
        ...
        FirebaseUtils.setValueWith2ChildObject(
            Constant.referenceRuangChat, chatLDtoAdmin, dbChat.idChat,
    ...
    private fun saveFoto(image : Uri, dataChat: ModelChat){
        ...
        FirebaseUtils.simpanFoto3Child(
            referenceChat, chatLDtoAdmin, dataChat.idChat, dataChat.timeStamp.toString(),
    ...
    private fun sendChat(dataChat: ModelChat) {
        ...
        FirebaseUtils.setValueWith3ChildObject(
            referenceChat, chatLDtoAdmin, dataChat.idChat,
        ...
        val onCompleteListener = OnCompleteListener<Void> {
            if (status == sended) {
                val notification = Notification(
                    ...
                    "Pesan"
                    , "com.exomatik.balligh.fcm_TARGET_NOTIFICATION_PESAN_LD_Admin"
                )
    ...
    private fun checkLastMessage(dataChat: ModelChat) {
        ...
        FirebaseUtils.getData2Child(
            ...
            chatLDtoAdmin, dataChat.idChat, valueEventListener)
    }
    ...
    private fun emptyMessage(dataChat: ModelChat) {
        ...
        FirebaseUtils.setValueWith3ChildString(
            Constant.referenceRuangChat, chatLDtoAdmin, dataChat.idChat,
            Constant.message, "",
            onCompleteListener, onFailureListener
        )
    }
